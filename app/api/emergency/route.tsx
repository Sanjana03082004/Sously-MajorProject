...
// 📨 GET → Decrypt and return all emergency contacts
export async function GET() {
  try {
    const session = await getServerSession(authOptions);
    if (!session?.user?.email) {
    ...
    const decryptedContacts = user.emergencyContacts.map((c: any) => ({
      ...
      name: decrypt(c.name),
      phone: decrypt(c.phone),
      email: decrypt(c.email),      // ✨ new field
      relation: decrypt(c.relation),
    }));
...
export async function POST(req: Request) {
  try {
    ...
    const email = session.user.email;
    const { name, phone, relation, email: contactEmail } = await req.json();

    const newContact = {
      _id: crypto.randomUUID(),
      name: encrypt(name),
      phone: encrypt(phone),
      email: encrypt(contactEmail),   // ✨ new field
      relation: encrypt(relation),
    };
    ...
    // Return decrypted for frontend display
    return NextResponse.json({
      ...newContact,
      name,
      phone,
